...
class BiDAF_baseline(nn.Module):
    ...
    def __init__(self, drop_prob=0.):
        super(BiDAF_baseline, self).__init__()
        hidden_size = 4096
        self.albert = AlbertModel.from_pretrained('albert-xxlarge-v2')
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, start_positions=None, end_positions=None):
        ...
        albert_out, _ = self.albert(input_ids=input_ids,
                                    attention_mask=attention_mask,
                                    token_type_ids=token_type_ids
                                    )
        q_emb = albert_out * q_mask[:, :, None].float()
        c_emb = albert_out * c_mask[:, :, None].float()
        # c_embs, c_masks = [], []
